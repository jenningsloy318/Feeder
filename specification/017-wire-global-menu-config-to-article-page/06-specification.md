...
**Document Version**: 2.2 (UPDATED with intelligent toolbar positioning)
**Date**: 2026-01-05
...
| 2.1 | 2026-01-05 | Added popup positioning fix (toolbar now appears below selection) |
| 2.2 | 2026-01-05 | Added intelligent toolbar positioning (prefers ABOVE first, like Android system) |
...
### 4.1 Popup Toolbar Positioning Fix

**Problem:**
- Initial implementation simply positioned toolbar below selection with fixed gap
- No consideration for available space above/below the selection
- Toolbar didn't adapt to screen boundaries (status bar, navigation bar)
- User feedback: "our implementation is not optimal, it is ugly"

**Root Cause:**
```kotlin
// INITIAL: Simple below positioning, no intelligence
val gapBelow = with(LocalDensity.current) { 8.dp.toPx() }.toInt()
...
**Result:** Toolbar now intelligently positions itself preferring above first (like Android system), with graceful fallback to below when needed. Matches system toolbar behavior.

---

### 4.2 Android 13+ Menu Not Showing
...
### 4.3 Third-Party Apps Receive No Text
